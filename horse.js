class Horse {
    ...
    draw(ctx) {
        ...
        // === COWBOY HAT ===
        const hatY = -103*s + breathe;
        const hatX = -28*s;
        // brim
        ctx.fillStyle = '#3a2510';
        ctx.beginPath();
        ctx.ellipse(hatX, hatY, 22*s, 6*s, -0.1, 0, Math.PI*2);
        ctx.fill();
        // crown
        ctx.fillStyle = '#4a3018';
        ctx.beginPath();
        ctx.moveTo(hatX - 12*s, hatY);
        ctx.quadraticCurveTo(hatX - 13*s, hatY - 18*s, hatX - 5*s, hatY - 20*s);
        ctx.quadraticCurveTo(hatX, hatY - 16*s, hatX + 5*s, hatY - 20*s);
        ctx.quadraticCurveTo(hatX + 13*s, hatY - 18*s, hatX + 12*s, hatY);
        ctx.fill();
        // hat band
        ctx.strokeStyle = '#c8a040';
        ctx.lineWidth = 2*s;
        ctx.beginPath();
        ctx.moveTo(hatX - 11*s, hatY - 5*s);
        ctx.quadraticCurveTo(hatX, hatY - 3*s, hatX + 11*s, hatY - 5*s);
        ctx.stroke();
